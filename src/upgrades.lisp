...
(defupgrades
  (:sturdy-gloves "Sturdy Gloves"
   "Double click mining power."
   :cost (:coins 25.0d0 :stone 20.0d0)
   :effect (setf (game-state-click-mult s) (* (game-state-click-mult s) 2.0d0))
   :unlock (>= (resource-ever s :stone) 20.0d0))

  (:miners-pouch "Miner's Pouch"
   "Double manual gathering yield."
   :cost (:coins 25.0d0 :stone 20.0d0)
   :effect (setf (game-state-gather-mult s) (* (game-state-gather-mult s) 2.0d0))
   :unlock (>= (resource-ever s :stone) 20.0d0))

  (:quick-hands "Quick Hands"
   "Manual mining and gathering are 0.5s faster."
   ...
   :unlock (>= (building-count s :pickaxe) 2))

  (:better-picks "Hardened Pickaxes"
   "Pickaxes mine 2× faster."
   :cost (:coins 100.0d0 :iron-b 5.0d0)
   :effect (multiply-building-prod! s :pickaxe 2.0d0)
   :unlock (>= (building-count s :pickaxe) 5))

  (:coal-veins "Deep Coal Seams"
   "Coal pits produce 50% more coal."
   :cost (:coins 300.0d0 :stone 50.0d0)
   :effect (multiply-building-prod! s :coal-pit 1.5d0)
   :unlock (>= (building-count s :coal-pit) 3))

  (:steel-alloy "Steel Alloy Research"
   "Unlocks steel smelting recipe."
   :cost (:coins 2000.0d0 :iron-b 20.0d0 :bronze 10.0d0)
   :unlock (>= (resource-ever s :iron-b) 20.0d0))

  (:smelt-slot-1 "Second Crucible"
   "Adds one extra smelting slot."
   :cost (:coins 800.0d0 :bronze 5.0d0)
   :effect (incf (game-state-smelt-slots-extra s))
   :unlock (>= (building-count s :furnace) 1))

  (:smelt-slot-2 "Third Crucible"
   "Adds one extra smelting slot."
   :cost (:coins 2500.0d0 :steel 5.0d0)
   :effect (incf (game-state-smelt-slots-extra s))
   :unlock (upgrade-owned-p s :smelt-slot-1))

  (:smelt-tuning "Furnace Tuning"
   "Smelting is 25% faster."
   :cost (:coins 500.0d0 :iron-b 10.0d0)
   :effect (setf (game-state-smelt-speed-mult s)
                 (* (game-state-smelt-speed-mult s) 1.25d0))
   :unlock (>= (resource-ever s :iron-b) 5.0d0))

  (:market-network "Market Network"
   "Market stalls generate 2× more coins."
   :cost (:coins 600.0d0 :copper-b 10.0d0)
   :effect (multiply-building-prod! s :market-stall 2.0d0)
   :unlock (>= (building-count s :market-stall) 1))

  (:deep-drilling "Deep-Core Drilling"
   "Unlocks Mythril Drill building."
   :cost (:coins 20000.0d0 :steel 20.0d0 :gear 10.0d0)
   :unlock (>= (resource-ever s :steel) 10.0d0))

  (:ancient-veins "Ancient Veins"
   ...
   :unlock (>= (resource-ever s :steel) 5.0d0))

  (:reinforced-sacks "Reinforced Sacks"
   "Stone/Coal storage +50%."
  ...
  (:ore-crates "Ore Crates"
   "Ore storage +50%."
   :cost (:coins 260.0d0 :gear 5.0d0 :bronze 5.0d0)
   :effect (dolist (kw '(:iron :copper :tin))
             (setf (gethash kw (game-state-caps s)) (* (resource-cap s kw) 1.5d0)))
   :unlock (>= (building-count s :iron-mine) 1))

  (:market-signage "Bright Signage"
   "Market stalls generate +50% coins."
   :cost (:coins 350.0d0 :copper-b 10.0d0 :stone 150.0d0)
   :effect (multiply-building-prod! s :market-stall 1.5d0)
   :unlock (>= (building-count s :market-stall) 1))

  (:precision-molds "Precision Molds"
   "Gears, bellows, and rivets craft 2× output."
   :cost (:coins 900.0d0 :bronze 20.0d0 :iron-b 20.0d0)
   :unlock (>= (resource-ever s :gear) 10.0d0))

  (:metalworking "Metalworking"
   "Unlocks Steel Plates and Rivets recipes."
   :cost (:coins 1800.0d0 :steel 5.0d0 :bronze 10.0d0)
   :unlock (>= (resource-ever s :steel) 5.0d0))

  (:coin-minting "Coin Minting"
   "Unlocks the Trade Token minting recipe."
   :cost (:coins 2500.0d0 :bronze 25.0d0 :gear 15.0d0)
   :unlock (>= (building-count s :market-stall) 2))

  (:industrial-tools "Industrial Tools"
   "Unlocks Machine Parts and boosts Mythril Drills +50%."
   :cost (:coins 15000.0d0 :steel-plate 10.0d0 :gear 20.0d0 :rivet 50.0d0)
   :effect (multiply-building-prod! s :mythril-drill 1.5d0)
   :unlock (>= (resource-ever s :steel-plate) 5.0d0))

  (:gear-driven-picks "Gear-Driven Picks"
   "Pickaxes mine 75% faster."
   :cost (:coins 1200.0d0 :gear 50.0d0)
   :effect (multiply-building-prod! s :pickaxe 1.75d0)
   :unlock (>= (building-count s :pickaxe) 20))

  (:coal-dust-filters "Coal Dust Filters"
   "Coal pits produce +75% coal."
   :cost (:coins 900.0d0 :bronze 12.0d0 :gear 6.0d0)
   :effect (multiply-building-prod! s :coal-pit 1.75d0)
   :unlock (>= (building-count s :coal-pit) 10))

  (:ore-sorting "Ore Sorting"
   "Iron/Copper mines and Tinsmith produce +40%."
   ...
   :unlock (>= (resource-ever s :bronze) 10.0d0))

  (:bellows-efficiency "Bellows Efficiency"
   "Bellows Workshops grant 15% smelt speed each."
   :cost (:coins 5000.0d0 :bellows 10.0d0 :steel 10.0d0)
   :unlock (>= (building-count s :bellows-shop) 1))

  (:assembly-lines "Assembly Lines"
   "All building production +15%."
   :cost (:coins 8000.0d0 :machine-part 3.0d0 :rivet 150.0d0 :steel-plate 15.0d0)
   :effect (setf (game-state-prod-mult s) (* (game-state-prod-mult s) 1.15d0))
   :unlock (>= (resource-ever s :machine-part) 1.0d0))

  (:mechanized-drills "Mechanized Drills"
   "Mythril Drills produce 2× mythril."
   :cost (:coins 75000.0d0 :machine-part 8.0d0 :steel 80.0d0)
   :effect (multiply-building-prod! s :mythril-drill 2.0d0)
